...
//       <div
//         ref={sidebarRef}
//         className={`fixed md:relative top-0 left-0 w-3/5 md:w-1/5 h-screen bg-[#431D5A] text-white md:p-4 p-2 shadow-md z-50 transition-transform duration-300 ${
...
//                     className={`flex-grow cursor-pointer md:p-2 p-[3px] text-xs md:text-[14px] rounded-sm ${
//                       chat.id === selectedId
...
//             className="md:w-3/6 w-[150px] mx-auto md:py-2 p-[2px] bg-red-500 hover:bg-[#ee5d5d] flex items-center justify-center gap-2 md:border-2 border border-white text-white rounded-full shadow-md text-sm  md:font-semibold font-medium transition-all duration-200"
//           >
//             Delete History
...
//           className="md:hidden fixed md:top-4 md:left-4 left-2 top-2 z-40 p-2 bg-[#431D5A] text-white rounded-md shadow-md"
//         >
...
function ChatHistory() {
  ...
  //     if (event.state === null) {
  //       // If there's no state, we're going back to the previous route
            ...
            {data.data.map((chat) => (
              <li key={chat.id} className="flex justify-between items-center md:p-2 p-1 border-b last:border-none">
                ...
                <div className="md:hidden dropdown dropdown-end">
                  ...
                  <ul
                    tabIndex={0}
                    className="dropdown-content border-none menu bg-[#48225f] z-[50] w-32 shadow-sm border-2 shadow-black/50 mt-2"
                  >
                    <button
                      onClick={() => handleEditClick(chat.id, chat.title)}
                      className="flex items-center space-x-2 mb-1 cursor-pointer hover:bg-[#734a8d] rounded-sm px-2 py-1"
                ...
                <div className="hidden md:block dropdown dropdown-end ">
                  ...
                  <ul
                    tabIndex={0}
                    className="dropdown-content border-none menu bg-[#48225f] z-[50] w-28 shadow-sm border-2 shadow-black/50 mt-2"
                  >
                    <button
                      onClick={() => handleEditClick(chat.id, chat.title)}
                      className="flex items-center space-x-2 mb-1 cursor-pointer hover:bg-[#734a8d] rounded-sm px-2 py-1"
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg md:w-[600px] px-1 text-center">
            ...
            <h3 className="md:text-2xl text-lg text-red-600 font-medium md:font-semibold mt-4">Are you sure?</h3>
            <p className="text-gray-600 mt-2 text-base">Do you really want to delete these records?</p>
            <p className="text-gray-600">This process cannot be undone.</p>
            <div className="flex justify-center space-x-4 mt-6">
              <button
                onClick={() => setIsDeleteModalOpen(false)}
